import { parseNfoMetadata } from '../utils/parsers/parseNfoMetadata'
import { readTextFile } from '../utils/fileUtils'

interface NfoLibraryFileObj {
  metadata: {
    path: string
  }
}
...
class NfoFileScanner {
   ...
   */
  async scanBookNfoFile(nfoLibraryFileObj: NfoLibraryFileObj, bookMetadata: Record<string, unknown>): Promise<void> {
    ...
    if (nfoMetadata) {
      for (const key in nfoMetadata) {
        ...
        } else if ((nfoMetadata as Record<string, unknown>)[key] && key !== 'sequence') {
          bookMetadata[key] = (nfoMetadata as Record<string, unknown>)[key]
        }
      }
    }
  }
