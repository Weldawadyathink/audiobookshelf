...
function parseFullName(
  nameToParse: string,
  partToReturn?: string,
  fixCase?: boolean | number | null,
  stopOnError?: boolean | number | null,
  useLongLists?: boolean | number | null
): ParsedName | string {
  var i, j, k, l, m, n, part, comma, titleList: string[], suffixList: string[], prefixList: string[], regex: RegExp,
    partToCheck: string, partFound: string[], partsFoundCount: number, firstComma: number, remainingCommas: number,
  ...
  function fixParsedNameCase(fixedCaseName: ParsedName, fixCaseNow: number | boolean | null): ParsedName {
    var forceCaseList = ['e', 'y', 'av', 'af', 'da', 'dal', 'de', 'del', 'der', 'di',
      'la', 'le', 'van', 'der', 'den', 'vel', 'von', 'II', 'III', 'IV', 'J.D.', 'LL.M.',
      'M.D.', 'D.O.', 'D.C.', 'Ph.D.']
    var forceCaseListIndex: number
    var namePartLabels: string[] = []
    var namePartWords: string[]
    if (fixCaseNow) {
  ...
  if (!nameToParse || typeof nameToParse !== 'string') {
    ...
    return partToReturn === 'all' ? parsedName : (parsedName[partToReturn] as string)
  } else {
  ...
  if (partsFound.length) {
    if (parsedName.suffix) {
      partsFound = [parsedName.suffix as string].concat(partsFound)
    }
    parsedName.suffix = partsFound.join(', ')
    partsFound = []
  }
