...
class OidcAuthStrategy {
   ...
   */
  async verifyCallback(tokenset: any, userinfo: any, done: Function) {
    let isNewUser = false
    let user = null
    try {
   ...
   * @returns {boolean}
   */
  validateGroupClaim(userinfo: any) {
   ...
   * @param {Object} userinfo
   */
  async setUserGroup(user: any, userinfo: any) {
    const groupClaimName = Database.serverSettings.authOpenIDGroupClaim
    if (!groupClaimName)
   ...
   * @param {Object} userinfo
   */
  async updateUserPermissions(user: any, userinfo: any) {
   ...
   */
  generatePkce(req: any, isMobileFlow: boolean) {
    if (isMobileFlow) {
   ...
   * @returns {string|null}
   */
  getEndSessionUrl(req: any, idToken: string, authMethod: string) {
    const client = this.getClient()
   ...
   */
  async getIssuerConfig(issuerUrl: string) {
   ...
   * @param {Response} res
   */
  handleMobileRedirect(req: any, res: any) {
    try {
   ...
   */
  isValidWebCallbackUrl(callbackUrl: string, req: any) {
