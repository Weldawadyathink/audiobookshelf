class AudioMetaTags {
  ...
  constructor(metadata?: Record<string, string | null | undefined> | null) {
    this.tagAlbum = null
  ...
  toJSON(): Record<string, string> {
    ...
    for (const key in this) {
      if (key.startsWith('tag') && this[key]) {
        json[key] = this[key] as string
      }
    }
    return json
  }

  get trackNumAndTotal(): { number: number | null; total: number | null } {
    const data = {
  ...
  get discNumAndTotal(): { number: number | null; total: number | null } {
    const data = {
      number: null as number | null,
      total: null as number | null
    }
  ...
  construct(metadata: Record<string, string | null | undefined>): void {
  ...
  setData(payload: Record<string, string | null | undefined>): void {
  ...
  updateData(payload: Record<string, string | null | undefined>): boolean {
    ...
    for (const key in dataMap) {
      if (dataMap[key] !== (this as Record<string, unknown>)[key]) {
        (this as unknown as Record<string, unknown>)[key] = dataMap[key]
        hasUpdates = true
      }
    }
    return hasUpdates
  ...
  isEqual(audioFileMetadata: AudioMetaTags | null): boolean {
    if (!audioFileMetadata || !audioFileMetadata.toJSON) return false
    for (const key in audioFileMetadata.toJSON()) {
      if ((audioFileMetadata as unknown as Record<string, unknown>)[key] !== (this as unknown as Record<string, unknown>)[key]) return false
    }
    return true
  }
}
